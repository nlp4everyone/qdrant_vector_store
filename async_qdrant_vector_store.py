...
# Inheritance
from qdrant_vector_store.base import AsyncBaseVectorStore
# Other components
...
class AsyncQdrantVectorStore(AsyncBaseVectorStore):
    ...
    async def retrieve(self,
        ...
        if mode == "dense":
            ...
            if isinstance(self._dense_embedding_model, BaseEmbedding):
                # Query embeddings
                embeddings = await self._dense_embedding_model.aget_query_embedding(query = query)
                # Model name
                model_name = list(dense_config.keys())[0]
            else:
                # Query embeddings
                embeddings = await self._dense_embedding_model.aembed_query(text = query)
                # Model name
                model_name = list(dense_config.keys())[0]
        else:
            # Sparse mode
            if self._sparse_embedding_model == None:
                raise ValueError("Please pass sparse embedding model!")
        ...
        scored_points = await self._client.query_points(collection_name = self._collection_name,
                                                        query = embeddings,
                                                        using = model_name,
    ...
    async def hybrid_query(self,
        ...
        if isinstance(self._dense_embedding_model, BaseEmbedding):
            # Query embeddings
            dense_embeddings = await self._dense_embedding_model.aget_query_embedding(query = query)
        else:
            # Query embeddings
            dense_embeddings = await self._dense_embedding_model.aembed_query(text = query)
        ...
        # Retrive points
        points = await self._client.query_points(collection_name = self._collection_name,
                                                 prefetch = [
                                                     models.Prefetch(
